<script>
  ...
  const handleCellEdit = (val, row, col, n) => {
    const params = Object.assign(row, {
      [col.key]: val.detail,
    });
    delete params.$expanded;
    delete params.$sortOn;
    console.log("dingo sending in the params", params);
    TableStore.setSelected(`${col.key}-${n}`, params);
  };
        ...
        >
          {#each columns as col}
            ...
            >
              ...
              {:else if editable}
                <TableInput
                  on:CELL_UPDATE={(val) => handleCellEdit(val, row, col, n)}
                  ID={`${col.key}-${n}`}
                  val={col.value(row)}
                />
              {:else}
